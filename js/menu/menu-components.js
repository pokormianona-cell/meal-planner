...
function generatePrompt() {
    ...
    var recentTitles = [];
    (appData.menuHistory || []).slice(0, 3).forEach(function(record) {
        (record.menu || []).forEach(function(meal) {
            if (meal.title && !recentTitles.includes(meal.title)) recentTitles.push(meal.title);
        });
    });
    var recentMealsBlock = recentTitles.length
        ? '\n\n=== НЕДАВНИЕ БЛЮДА — НЕ ПОВТОРЯТЬ ===\nНе предлагай эти блюда снова и не делай их косметические вариации с другим гарниром или соусом:\n- ' + recentTitles.slice(0, 30).join('\n- ')
        : '';
    
    return 'Привет! Ты — шеф-повар, который делает домашнюю еду захватывающей, вкусной и реалистичной в приготовлении. Составь меню на ' + getWeekDateRange() + '.\n\n=== АКТУАЛЬНЫЕ ПРОДУКТЫ В НАЛИЧИИ ===\nНиже добавлен мой актуальный список продуктов. В первую очередь строй меню вокруг него и явно учитывай указанное количество. Новые ингредиенты предлагай только когда они действительно улучшают блюдо.\n' + productsList + '\n\n=== НУЖНО МЕНЮ ===\n' + selectedDaysList + recentMealsBlock + '\n\n=== ОБО МНЕ ===\nЖенщина, ' + appData.userHeight + 'см/' + appData.userWeight + 'кг, ' + dailyCalories + 'ккал/день (включая колу 140 ккал).\nЦели: завтрак ~' + bCal + ' ккал, обед ~' + lCal + ' ккал, ужин ~' + dCal + ' ккал.\nЯ готовлю каждый приём пищи. Готовлю пока неуверенно, но хочу учиться: давай понятные пошаговые рецепты, объясняй важные приёмы и постепенно знакомь с новыми техниками, без ресторанной сложности.\nЗавтраки должны быть быстрыми; в обеде и ужине обязательно достаточно белка. На ужин я чаще предпочитаю салаты, но это пожелание, а не жёсткое правило. Хлеб не покупаю. Я НЕ ЛЮБЛЮ СМУЗИ И БОУЛЫ.\nЕсть аэрогриль, вафельница, плита, духовка, блендер, вакууматор, сувид и кокотницы объёмом 250 мл.\n\n=== ПРИОРИТЕТ: РАЗНООБРАЗИЕ И ИНТЕРЕСНАЯ ЕДА ===\nКБЖУ — обязательное ограничение, но не источник идеи блюда. Сначала придумай аппетитное и самобытное блюдо, затем подгони порцию под целевые КБЖУ. Не своди меню к одинаковому белку с разными гарнирами.\n\nРаспредели блюда по разным кулинарным характерам: используй кухни разных стран, разные основные продукты, гарниры, соусы, специи, текстуры и способы приготовления. Не ставь рядом два похожих блюда. Не повторяй одну и ту же комбинацию «основной продукт + база + соус» и не используй один способ приготовления больше двух раз, если есть разумная альтернатива. Повтор основного белка допустим, но вкус, техника и форма блюда должны заметно отличаться.\n\nУ каждого блюда должна быть одна понятная фишка, ради которой его хочется приготовить: яркий соус, необычная специя, контраст хрустящего и нежного, интересная начинка, маринад или новая техника. Фишка должна быть логичной, отражаться в названии и реально выполняться в рецепте — без случайных ресторанных украшений и редких ингредиентов ради эффектности.\n\nРецепты делай конкретными и обучающими: указывай температуру, примерное время, размер нарезки, признаки готовности и важный приём там, где это помогает не ошибиться. Каждый шаг — отдельное понятное действие.\n\n=== КБЖУ И ПРОВЕРКА ===\nДля каждого блюда точно рассчитай калории, белки, жиры и углеводы по ингредиентам. Итог каждого приёма пищи должен быть близок к его цели (допуск ±10%). Не выдумывай значения: сумма КБЖУ ингредиентов должна совпадать с total.\n\nПеред ответом молча проверь всё меню: блюда не похожи друг на друга и на недавние; названия отражают их фишку; шаги действительно позволяют приготовить блюдо; количества реалистичны; суммы КБЖУ совпадают.\n\n=== ФОРМАТ JSON ===\nВерни только валидный JSON-массив без пояснений. Используй сокращения дней: Пн, Вт, Ср, Чт, Пт, Сб, Вс.\n[{"day":"Пн","meal":"Завтрак","title":"...","recipe":["..."],"ingredients":[{"name":"...","amount":"100г","kcal":120,"protein":10,"fat":5,"carbs":8}],"total":{"kcal":' + bCal + ',"protein":30,"fat":20,"carbs":50}}]';
}
