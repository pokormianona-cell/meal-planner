/*
   menu-subtract.js - очищенная версия
*/
...
function findMatchingProduct(name, products) {
    ...
    let m = products.find(p => String(p.name).toLowerCase().includes(s) || s.includes(String(p.name).toLowerCase()));
    if (m) return m;
    const kw = s.split(' ').filter(w => w.length > 2);
    for (let k of kw) { m = products.find(p => String(p.name).toLowerCase().includes(k)); if (m) return m; }
    return null;
}
...
function calculateSubtractAmount(required, requiredUnit, product) {
    ...
    if ((rn === 'г' || rn === 'мл') && (product.unitWeight || product.weight) > 0) {
        return required / (product.unitWeight || product.weight);
    }
...
function prepareSubtractData(mealInfo) {
    ...
    for (const ing of fullMeal.ingredients) {
        ...
        if (!product) { items.push({ ingredient: name, required: amount, found: false }); continue; }
        ...
        items.push({
            ...
            subtractValue: subVal, isEnough: product.amount >= subVal, found: true
        });
    }
...
function updateProductSelection(index, val) {
    ...
    if (!val) { 
        item.selectedProductId = null; item.found = false; 
        ...
        const p = appData.products.find(x => x.id == val);
        if (p) {
            item.selectedProductId = p.id; item.product = p; item.productName = p.name; 
            item.productAmount = p.amount; item.productUnit = p.unit; item.found = true;
...
async function confirmSubtract() { 
    ...
    if(pendingRating){ const r = pendingRating; pendingRating = null; if(typeof openRatingModal === 'function') openRatingModal(r.mealInfo, r.liked); } 
}
...
function skipSubtractAndRate() { 
    ...
    closeSubtractModal(); 
    if(mealInfo){ mealInfo.cooked = true; mealInfo.cookedDate = new Date().toISOString(); dbSaveMenu(appData.parsedMenu, appData.weekStartDate); if(typeof displayMenu === 'function') displayMenu(appData.parsedMenu); if(p) openRatingModal(p.mealInfo, p.liked); } 
}

function addCustomItem() { 
    if(!currentSubtractData) return; 
    const item = { ingredient: '', required: '', product: null, selectedProductId: null, found: false, subtractValue: 0, isEnough: false }; 
...
async function markAsCooked(mealInfo) {
    if (!mealInfo) return;
    const fullMeal = appData.parsedMenu?.find(m => m.day === mealInfo.day && m.meal === mealInfo.meal && m.title === mealInfo.title);
    if (!fullMeal) { console.error('Блюдо не найдено'); return; }
    if (fullMeal.cooked) { return; }
    const data = prepareSubtractData(mealInfo);
    if (!data.items.length) { fullMeal.cooked = true; fullMeal.cookedDate = new Date().toISOString(); await dbSaveMenu(appData.parsedMenu, appData.weekStartDate); displayMenu(appData.parsedMenu); return; }
...
function markAsCookedAndRate(mealInfo, liked) {
    ...
    const data = prepareSubtractData(mealInfo);
    if (!data.items.length) { fullMeal.cooked = true; fullMeal.cookedDate = new Date().toISOString(); dbSaveMenu(appData.parsedMenu, appData.weekStartDate); displayMenu(appData.parsedMenu); if (typeof openRatingModal === 'function') openRatingModal(mealInfo, liked); return; }
